...
extern	BOOLEAN	gfAlternateRoofHiding;
...
void MonitorEditorMessage( void );
...
UINT32 EditScreenInit(void)
{
	...
	//Init the selection types
	swprintf( wszSelType[0], gzEditorStrings[EDITOR_STR_SMALL] );
	swprintf( wszSelType[1], gzEditorStrings[EDITOR_STR_MEDIUM] );
	swprintf( wszSelType[2], gzEditorStrings[EDITOR_STR_LARGE] );
	swprintf( wszSelType[3], gzEditorStrings[EDITOR_STR_XLARGE] );
	swprintf( wszSelType[4], gzEditorStrings[EDITOR_STR_WIDTH] );
	swprintf( wszSelType[5], gzEditorStrings[EDITOR_STR_AREA] );
...
BOOLEAN EditModeInit( void )
{
	...
	if ( !gfWorldLoaded )
	{
		OutputDebugString( "Creating initial world...\n" );
		CreateNewMap( );
	}
		...
		if( !HandleSummaryInput( &EditorInputEvent ) && !HandleTextInput( &EditorInputEvent ) && EditorInputEvent.usEvent == KEY_DOWN )
		{
			...
			else switch( EditorInputEvent.usParam )
			{
				...
				case ENTER:
					...
					else if( iCurrentTaskbar == TASK_OPTIONS )
				...
				case DEL:
					...
					else if( gsSelectedMercID != -1 )
					{
						...
						else if ( gubCurrMercMode == MERC_GETITEMMODE )
						{	
				...
				case 'h':
					if( EditorInputEvent.usKeyState & ALT_DOWN )
					{
						gfAlternateRoofHiding = TRUE;
					}
					else
					{
						gfAlternateRoofHiding = FALSE;
					}
...
void CreateNewMap()
{
	...
	memset( &gMapInformation, 0, sizeof( gMapInformation ) );
...
void EnsureStatusOfEditorButtons()
{
	...
	if( iDrawMode == DRAW_MODE_CAVES || iDrawMode == DRAW_MODE_NEWROOF ||
			iDrawMode == DRAW_MODE_SMART_WALLS || iDrawMode == DRAW_MODE_SMART_WINDOWS ||
			iDrawMode == DRAW_MODE_SMART_DOORS || iDrawMode == DRAW_MODE_SMART_BROKEN_WALLS ||
			iDrawMode == DRAW_MODE_ROOM || iDrawMode == DRAW_MODE_NEWROOF || iDrawMode == DRAW_MODE_SAW_ROOM ||
			iDrawMode == DRAW_MODE_KILL_BUILDING || iDrawMode == DRAW_MODE_COPY_BUILDING || iDrawMode == DRAW_MODE_MOVE_BUILDING ||
	...
	if( _LeftButtonDown )
	{
		...
		switch ( iDrawMode )
		{
			...
			case DRAW_MODE_ROADS:
				...
				{ //not the cave tileset, do road macros
				...
				else
				{
					...
					pNumSelList = &iNumRoadsSelected;
					usUseIndex = pSelList[ iCurBank ].usIndex;
					PasteBanks( iMapIndex, usUseIndex, TRUE );
				}
				break;
			case (DRAW_MODE_GROUND + DRAW_MODE_FILL_AREA):
...
UINT32  EditScreenHandle( void )
{
	...
	if( gfWorldLoaded && gubScheduleID > 40 && !gfCorruptSchedules )
	{
		...
		if( gubScheduleID > 32 )
		{
			ScreenMsg( FONT_MCOLOR_RED, MSG_ERROR, L"Schedule data has just been corrupted. " );
	...
	if ( InOverheadMap( ) && !gfSummaryWindowActive && !gfStartWindowActive )
	{
		HandleOverheadMap( );
	}
	...
	if( !gfGotoGridNoUI && iDrawMode != DRAW_MODE_SHOW_TILESET && !gfSummaryWindowActive && !gfStartWindowActive &&
	...
	if( !gfSummaryWindowActive && !gfEditingDoor && !InOverheadMap() && !gfStartWindowActive && !gfStartWindowActive )
		RenderWorld();
...
//		if( !gfInConfirmMapMoveMode || bSelectedDestChar == -1 )
		{
			...
			if ( ( GetJA2Clock( ) - guiUIMessageTime ) > 2000 )
			{
