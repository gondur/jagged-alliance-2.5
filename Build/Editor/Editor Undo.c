...
extern void ResetItemEditingSelection( );
...
typedef struct 
{
	...
	UINT8					ubRoomNum;
	BOOLEAN				fNorthEntryPoint;
	BOOLEAN				fSouthEntryPoint;
	BOOLEAN				fEastEntryPoint;
	BOOLEAN				fWestEntryPoint;
	BOOLEAN				fCenterEntryPoint;
...
BOOLEAN AddToUndoListCmd( INT32 iMapIndex, INT32 iCmdCount, INT8 bEntryPoint );
...
BOOLEAN AddToUndoList( INT32 iMapIndex )
{
	...
	{
		if( AddToUndoListCmd( iMapIndex, ++iCount, 0 ) )
			return TRUE;
		iCount--;
	}
	return FALSE;
...
BOOLEAN AddEntryPointToUndoList( INT32 iMapIndex, INT8 bEntryPoint )
{
	static INT32 iCount = 1;
	...
	{
		if( AddToUndoListCmd( iMapIndex, ++iCount, bEntryPoint ) )
			return TRUE;
		iCount--;
	}
	return FALSE;
...
BOOLEAN AddToUndoListCmd( INT32 iMapIndex, INT32 iCmdCount, INT8 bEntryPoint )
{
	...
	{
		case 1:
			pUndoInfo->fNorthEntryPoint = TRUE;
			break;

		case 2:
			pUndoInfo->fWestEntryPoint = TRUE;
			break;

		case 3:
			pUndoInfo->fEastEntryPoint = TRUE;
			break;

		case 4:
			pUndoInfo->fSouthEntryPoint = TRUE;
			break;

		case 5:
			pUndoInfo->fCenterEntryPoint = TRUE;
			break;
...
BOOLEAN ExecuteUndoList( void )
{
	...
	while ( (iCurCount < iCmdCount) && (gpTileUndoStack != NULL) )
	{
		...
		else if ( gpTileUndoStack->pData->fNorthEntryPoint )
		{
			gMapInformation.sNorthGridNo = -1;
		}
		else if ( gpTileUndoStack->pData->fSouthEntryPoint )
		{
			gMapInformation.sSouthGridNo = -1;
		}
		else if ( gpTileUndoStack->pData->fEastEntryPoint )
		{
			gMapInformation.sEastGridNo = -1;
		}
		else if ( gpTileUndoStack->pData->fWestEntryPoint )
		{
			gMapInformation.sWestGridNo = -1;
		}
		else if ( gpTileUndoStack->pData->fCenterEntryPoint )
		{
...
BOOLEAN CopyMapElementFromWorld( MAP_ELEMENT *pNewMapElement, INT32 iMapIndex )
{
	...
	for( x = 0; x < 9; x++ )
	{
		...
		while( pOldLevelNode )
		{
			// ATE: Don't copy over item pool information...
...
extern LEVELNODE *AddItemGraphicToWorld( INVTYPE *pItem, INT16 sGridNo, UINT8 ubLevel );
...
BOOLEAN SwapMapElementWithWorld( INT32 iMapIndex, MAP_ELEMENT *pUndoMapElement )
{
	...
	// OK, to maintain items, look for any world items at this gridno
	// and add their graphic again, since undo dosn't change items

	// Create a list of items 
	pObjects = MemAlloc( sizeof( WORLDITEM ) * 100 );

	for( uiCount = 0; uiCount < guiNumWorldItems; uiCount++ )
	{
		if( gWorldItems[ uiCount ].fExists )
		{
			if ( gWorldItems[ uiCount ].sGridNo == iMapIndex )
			{
				if ( uiNumItems < 100 )
				{
					pObjects[ uiNumItems ] = gWorldItems[ uiCount ];
					uiNumItems++;
				}
				else
				{
	...
	// Remove item pool from this node
	RemoveItemPool( (UINT16)iMapIndex, 0 );
	RemoveItemPool( (UINT16)iMapIndex, 1 );
	...
	*pUndoMapElement = TempMapElement;
	...
	// FInally, add again, if there were items
	for ( uiCount = 0; uiCount < uiNumItems; uiCount++ )
	{
		// Add the ho!
		AddItemToPool( pObjects[ uiCount ].sGridNo, &(pObjects[ uiCount ].o ), TRUE, pObjects[ uiCount ].ubLevel, pObjects[ uiCount ].usFlags, pObjects[ uiCount ].bRenderZHeightAboveLevel );
	}

	MemFree( pObjects );

	// Finally, now, reset some editor globals incase we just
	// foobared the current one
	//ResetEditorSelectedItem( );
