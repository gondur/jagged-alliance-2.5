...
void RenderMapEntryPointsAndLights()
{
	...
	INT32 i;
	if( gfSummaryWindowActive || gfStartWindowActive )
		return;
	SetFont( FONT10ARIAL );
	SetFontForeground( FONT_YELLOW );
	...
	for( i = 0; i < MAX_LIGHT_SPRITES; i++ )
	{
		if( LightSprites[ i ].uiFlags & LIGHT_SPR_ACTIVE )
		{
			...
			GetGridNoScreenPos( sGridNo, 0, &sScreenX, &sScreenY );
			if( sScreenY >= -50 && sScreenY < 350 && sScreenX >= -40  && sScreenX < SCREEN_BUFFER_WIDTH )
			{
...
void BuildTriggerName( OBJECTTYPE *pItem, UINT16 *szItemName )
{
	if( pItem->usItem == SWITCH )
	{
		if( pItem->bFrequency == PANIC_FREQUENCY )
			...
			swprintf( szItemName, gzEditorStrings[EDITOR_STR_PANIC4] );
		else if( pItem->bFrequency == PANIC_FREQUENCY_5 )
			swprintf( szItemName, gzEditorStrings[EDITOR_STR_PANIC5] );
		else if( pItem->bFrequency == PANIC_FREQUENCY_6 )
			swprintf( szItemName, gzEditorStrings[EDITOR_STR_PANIC6] );
		else
	...
	{ //action item
		if( pItem->bDetonatorType == BOMB_PRESSURE )
			...
			swprintf( szItemName, gzEditorStrings[EDITOR_STR_PANIC_ACTION4] );
		else if( pItem->bFrequency == PANIC_FREQUENCY_5 )
			swprintf( szItemName, gzEditorStrings[EDITOR_STR_PANIC_ACTION5] );
		else if( pItem->bFrequency == PANIC_FREQUENCY_6 )
			swprintf( szItemName, gzEditorStrings[EDITOR_STR_PANIC_ACITON6] );
		else
...
void RenderSelectedItemBlownUp()
{
	...
	// ATE: Don't assert here , we could have deleted the item
	if ( pItemPool == NULL )
	{
		return;
	}
...
void RenderEditorInfo( )
{
	...
	mprintfEditor( (UINT16)5, 463, FPSText );
	...
	switch( iCurrentTaskbar )
	{
		...
		case TASK_TERRAIN:
			...
			swprintf( FPSText, L"%3d%%", gusSelectionDensity );
		...
		case TASK_MAPINFO:
			...
			if( gusSelectionType == LINESELECTION )
				swprintf( wszSelType[LINESELECTION], gzEditorStrings[EDITOR_STR_WIDTH_INDICATOR],gusSelectionWidth );
			DrawEditorInfoBox( wszSelType[gusSelectionType], FONT12POINT1, 340, 430, 60, 30 );
			break;
	}
}
...
void ProcessEditorRendering()
{
	...
	if( !gfSummaryWindowActive && !gfGotoGridNoUI && !InOverheadMap() && !gfStartWindowActive )
	{
