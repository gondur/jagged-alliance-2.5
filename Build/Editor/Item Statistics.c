...
void UpdateItemStatsPanel()
{
	...
	switch( gbEditingMode )
	{
		...
		case EDITING_TRIGGERS:
			...
			if( gpEditingItemPool && gpItem->bFrequency >= PANIC_FREQUENCY_6 && gpItem->bFrequency <= PANIC_FREQUENCY )
			{
	...
	if( gpEditingItemPool )
	{
		...
		// IAN COMMENTED THIS OUT OCT 11, 2000 AS IT'S NOT RELEVANT
		//mprintf( 587, 366, L"B" );
		//mprintf( 609, 366, L"R" );
		//mprintf( 630, 366, L"S" );
	}
...
void SetupTriggersGUI()
{
	...
	if( gpEditingItemPool )
	{
		swprintf( str, L"%d", 100 - gWorldItems[ gpEditingItemPool->iItemIndex ].ubNonExistChance );
		AddTextInputField( 485, 440, 25, 15, MSYS_PRIORITY_NORMAL, str, 3, INPUTTYPE_NUMERICSTRICT );
		if( gpItem->bFrequency <= PANIC_FREQUENCY && gpItem->bFrequency >= PANIC_FREQUENCY_6 )
		{
...
void RemoveTriggersGUI()
{
	if( gpEditingItemPool && gpItem->bFrequency <= PANIC_FREQUENCY && gpItem->bFrequency >= PANIC_FREQUENCY_6 )
	{
		if( giAlarmTriggerButton != -1 )
		{
			RemoveButton( giAlarmTriggerButton );
...
//			pItem->usBombItem = NOTHING;
//			pItem->bActionValue = ACTION_ITEM_BIGGENS_BOMBS;
//			break;
//		case ACTIONITEM_BIGGENS_WARNING:
//			pItem->usBombItem = NOTHING;
//			pItem->bActionValue = ACTION_ITEM_BIGGENS_WARNING;
//			break;
//		case ACTIONITEM_SEE_FORTIFIED_DOOR:
//			pItem->usBombItem = NOTHING;
//			pItem->bActionValue = ACTION_ITEM_SEE_FORTIFIED_DOOR;
//			break;
//		case ACTIONITEM_OPEN_FORTIFED_DOOR:
//			pItem->usBombItem = NOTHING;
//			pItem->bActionValue = ACTION_ITEM_OPEN_FORTIFED_DOOR;
//			break;
//		case ACTIONITEM_SEE_POWER_GEN_FAN:
//			pItem->usBombItem = NOTHING;
//			pItem->bActionValue = ACTION_ITEM_SEE_POWER_GEN_FAN;
//			break;
