...
void gh( GUI_BUTTON *btn, INT32 reason )
{
	if( reason & MSYS_CALLBACK_REASON_LBUTTON_UP )
	{
		gfRenderMercInfo = TRUE;
...
void SpecifyEntryPoint( UINT32 iMapIndex )
{
	...
	if( !fErasing )
	{
		...
		*psEntryGridNo = (UINT16)iMapIndex;
		...
		AddEntryPointToUndoList( *psEntryGridNo , bEntryPoint );
...
void UpdateMercsInfo()
{
	...
	switch( gubCurrMercMode )
	{
		...
		case MERC_GENERALMODE:
			...
			mprintf( 240, 361, gzEditorStrings[EDITOR_STR_ORDERS_LABEL]);
			mprintf( 240, 417, gzEditorStrings[EDITOR_STR_ATTITUDE_LABEL]);
			if( iDrawMode == DRAW_MODE_CREATURE )
			{
				DisplayBodyTypeInfo();
				SetFont( SMALLCOMPFONT );
			...
			if( iDrawMode == DRAW_MODE_ENEMY )
			{
				SetFont( FONT10ARIAL );
				SetFontForeground( FONT_YELLOW );
				mprintf( 570, 430, gzEditorStrings[EDITOR_STR_ARMY] );
				mprintf( 570, 416, gzEditorStrings[EDITOR_STR_ADMIN] );
				mprintf( 570, 444, gzEditorStrings[EDITOR_STR_ELITE] );

				mprintf( 540, 405, gzEditorStrings[EDITOR_STR_RANK_LABEL] );				
			}

			if ( gpSelected )
			{
				if ( gpSelected->pBasicPlacement->bBodyType == BLOODCAT )
				{
					DisableButton( iEditorButton[ MERCS_INVENTORY ] );
				}
				else
				{
					EnableButton( iEditorButton[ MERCS_INVENTORY ] );
				}
			}
...
void RenderMercInventoryPanel()
{
...

	{
		...
		gprintfdirty( 178, 361, gzEditorStrings[ EDITOR_STR_CHECK ] );
		mprintf( 178, 361, gzEditorStrings[ EDITOR_STR_CHECK ] );

		gprintfdirty( 178, 371, gzEditorStrings[ EDITOR_STR_DROPPABLE ] );
		mprintf( 178, 371, gzEditorStrings[ EDITOR_STR_DROPPABLE ] );

		gprintfdirty( 178, 381, gzEditorStrings[ EDITOR_STR_DROP_ITEMS ] );
		mprintf( 178, 381, gzEditorStrings[ EDITOR_STR_DROP_ITEMS ] );
