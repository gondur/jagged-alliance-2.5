...
void RenderStationaryGroupsUnderGround();
void HandleAiViewerDifficuyltyModifier();
...
UINT32 guiNumberAdditionalEnemies=0;
...
//JA25	if( gfViewEnemies && !gbViewLevel )
	if( gfViewEnemies )
	{
		if( gbViewLevel )
			RenderStationaryGroupsUnderGround();
		else
		{
		...
		SetFontForeground( FONT_LTRED );
//ja25		mprintf( 78, 358, L"%3d", giReinforcementPool );
...
void RenderViewer()
{
			...
			for( y = 0; y < 16; y++ )
			{
				...
				for( x = 0; x < 16; x++ )
				{
					...
					if( pUnder )
					{
						...
						//ja25mprintf( xp, yp, L"%d", pUnder->ubNumCreatures );
					}
					else
	...
	{
		CHAR16 zString[512];
		swprintf( zString, L"Additional Enemies: %d", guiNumberAdditionalEnemies );
		ClearViewerRegion( 280, 466, 450, 480 );
		DrawTextToScreen( zString, 280, 466, 0, FONT10ARIAL, FONT_YELLOW, 0, FALSE, INVALIDATE_TEXT | LEFT_JUSTIFIED );
	}
...
*/
		...
		InitStrategicLayer();
		HandleAiViewerDifficuyltyModifier();
...
void ExtractAndUpdatePopulations()
{
	...
	SetNumberJa25EnemiesInSector( gsSelSectorX, gsSelSectorY, gbViewLevel, (UINT8)gsAINumAdmins, (UINT8)gsAINumTroops, (UINT8)gsAINumElites );
}
...
void PrintEnemyPopTable()
{
	...
	SECTORINFO *pSector;
	UNDERGROUND_SECTORINFO *pUSector;
	...
	pUSector = gpUndergroundSectorInfoHead;
	while( pUSector )
	{
		usEnemyPopTable[ ENEMY_RANK_ADMIN ][ ENEMY_TYPE_GARRISON ] += pUSector->ubNumAdmins;
		usEnemyPopTable[ ENEMY_RANK_TROOP ][ ENEMY_TYPE_GARRISON ] += pUSector->ubNumTroops;
		usEnemyPopTable[ ENEMY_RANK_ELITE ][ ENEMY_TYPE_GARRISON ] += pUSector->ubNumElites;

		pUSector = pUSector->next;
	}
...
void RenderStationaryGroupsUnderGround()
{
	...
	pSector = gpUndergroundSectorInfoHead;

	while( pSector )
	{
		if( pSector->ubSectorZ != gbViewLevel )
		{
			pSector = pSector->next;
			continue;
		}

		yp = VIEWER_TOP + VIEWER_CELLH * ( pSector->ubSectorY - 1 );

			SetFontForeground( FONT_YELLOW );
			xp = VIEWER_LEFT + VIEWER_CELLW * ( pSector->ubSectorX - 1 );
/*
			if( pSector->uiFlags & SF_MINING_SITE )
				BltVideoObject( FRAME_BUFFER, hVObject, MINING_ICON, xp + 25, yp - 1, VO_BLT_SRCTRANSPARENCY, NULL );
...
void HandleAiViewerDifficuyltyModifier()
{
	if( gfKeyState[ ALT ] )
	{
		guiNumberAdditionalEnemies = HandleDifficultyModifierForImportedGames();
	}
	else
	{
		guiNumberAdditionalEnemies=0;
	}
}
