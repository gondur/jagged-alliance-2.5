...
//	gp12PointArialFixedFont = LoadFontFile( "FONTS\\Arial12FixedWidth.sti" );
  ...
  wcscpy( zWinFontName, L"Arial" );

  if ( !DoesWinFontExistOnSystem( zWinFontName ) )
  {
		FatalError( "Cannot load subtitle Windows Font: %S.", zWinFontName );
    return( FALSE );
  }
  else
  {
    giSubTitleWinFont = CreateWinFont( 14, 0, 0,  0, FALSE, FALSE, FALSE, zWinFontName );
  }
#endif
